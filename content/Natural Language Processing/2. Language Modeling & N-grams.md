...
$$P_{Laplace}(w_i) = {c_i + 1 \over N + V} \text{ or } {P_{Laplace}(w_i \mid w_{i-1}) = {count(w_{i-1}, w_i) + 1 \over count(w_{i-1}) + V}}$$
...
**Add-k Smoothing**
$${P_{add-k}(w_i \mid w_{i-1}) = {count(w_{i-1}, w_i) + k \over count(w_{i-1}) + kV}}$$

**Unigram Prior Smoothing**
$${P_{unigram\ prior}(w_i \mid w_{i-1}) = {count(w_{i-1}, w_i) + mP(w_i) \over count(w_{i-1}) + m}}$$
