...
4. **Standard Task Execution:** If `.raw_sources` is empty, proceed directly to fulfilling the given task. Always rely on `[[index]]` (from content/index.md) and MOCs to locate relevant notes and understand where to operate.
...
7. **Index Maintenance:** Update `[[index]]` only when creating new top-level folders/domains or major structural patterns.

### Core Search (Discovery) Workflow
1. **Navigate via INDEX & MOCs:** Always consult `[[index]]` first to locate the relevant domain, then check that domain's MOC (e.g., `[[Machine Learning MOC]]`) to find specific notes or understand the logical structure of the topic.
...
- **Folder Consolidation:** As domains grow and overlap, proactively suggest merging smaller top-level folders into broader categories to prevent directory bloat. This involves moving files, combining their respective MOCs, and updating `INDEX.md`.
...
- **index.md:** Maintain this as a lightweight map. Update when new folders or major patterns emerge.
