...
4. **Standard Task Execution:** If `.raw_sources` is empty, proceed directly to fulfilling the given task, and still update the knowledgebase. Always add or refine relevant notes in `content/` based on the request (or identify and fill gaps), and rely on `[[toc]]` (from content/toc.md) and MOCs to locate relevant notes and understand where to operate.
